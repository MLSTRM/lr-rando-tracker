...
export const QuestPrerequisites: { [key: string]: { name: string; check: (context: Partial<QuestProgressCheck>) => boolean } } = {
    ...
    "dunes_first_tablet": { name: 'First Tablet', check: context => (context.mainQuestBytes?.deaddunes ?? 0) >= 800 },
    "dunes_second_tablet": { name: 'Second Tablet', check: context => (context.mainQuestBytes?.deaddunes ?? 0) >= 900 },
    "dunes_third_tablet": { name: 'Third Tablet', check: context => (context.mainQuestBytes?.deaddunes ?? 0) >= 1000 },
